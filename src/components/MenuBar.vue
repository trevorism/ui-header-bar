  ...
  <va-navbar v-bind="blackColor" class="header" style="height:70px">
      ...
      <va-navbar-item v-bind:key="item.name" v-for="item in leftMenuBar">
        <div v-if="item.hasOwnProperty('children')">
          <va-button-dropdown :label="item.name" size="medium" class="mr-2 mb-2">
            <multi-menu-link-item v-bind:key="child.name" v-for="child in item.children" :href="child.link" :local="local">
              {{ child.name }}
    ...
    <template #right>
      <va-navbar-item v-bind:key="item.name" v-for="item in rightMenuBar">
        <div v-if="item.hasOwnProperty('children')">
          <va-button-dropdown :label="item.name" size="medium" class="mr-2 mb-2">
            <multi-menu-link-item v-bind:key="child.name" v-for="child in item.children" :href="child.link"
                                  :local="local">{{ child.name }}
            </multi-menu-link-item>
          </va-button-dropdown>
        </div>
        <div v-else>
...
const articles = {
  "name": "Articles", "children": [
    ...
    { "name": "Prototype Driven Development 05/2013", "link": link("/articles/prototype") },
    { "name": "Technology Trends 02/2018", "link": link("/articles/trends") },
    { "name": "Productionalized Service 02/2018", "link": link("/articles/production") },
    { "name": "Scientific Improvement 12/2020", "link": link("/articles/improvement") }
  ]
};
...
const leftMenuBar = computed(() => {
  let arr = [apps, articles, contact, tools];
